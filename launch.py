...
def execute_remote(
    ...
    # Construct ssh command that executes `cmd` on the remote host
    ssh_cmd = (
    f"ssh -o StrictHostKeyChecking=no -p {port} {ip_prefix}{ip} "
    f"'export LD_LIBRARY_PATH={ld_library_path}; {cmd}'"
    )
...
def wrap_udf_in_torch_dist_launcher(
    ...
    if nsys_str in udf_command:
        # start split in udf command by space
        udf_command_split = udf_command.split(python_bin)
        script = udf_command_split[1]
        nsys_python_bin = udf_command_split[0]
        nsys_python_bin_splits = nsys_python_bin.split(' ')
        python_path = nsys_python_bin_splits[-1]
        nsys_part = nsys_python_bin_splits[:-1]
        # print(nsys_part)

        # extract from nsys part the string after -o and append id to it
        #extract path except for last part
        output_path = nsys_part[nsys_part.index('-o') + 1].split('/')[:-1]
        # print("output_path", output_path)
        output_path = '/'.join(output_path) # join the path back together
        output_name = nsys_part[nsys_part.index('-o') + 1].split('/')[-1] # filename
